                    ...
                    <motion.div layout className="flex flex-col gap-4 w-[292px] shrink-0 h-full">
                        ...
                        <motion.div layout className="mt-1 flex items-center gap-0 w-full p-1 bg-white/[0.02] border border-white/5 rounded-[18px] backdrop-blur-md">
                            ...
                            <GlassButton
                                variant="primary"
                                size="sm"
                                id={isActive ? "demo-job-apply-btn" : undefined}
                                ...
                                onClick={() => {
                                    const demoParam = typeof window !== "undefined" && window.location.search.includes("demo=1") ? "&demo=1" : "";
                                    router.push(`/jobs/apply?role=${role.id}${demoParam}`);
                                }}
                            >
                                {t.applyNow}
                            </GlassButton>
...
export default function CareersPage() {
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && window.location.search.includes("demo=1")) {
            setIsDemoMode(true);
            const runDemo = async () => {
                // Wait for page to render and header animation
                await new Promise(r => setTimeout(r, 1500));

                // Scroll down to the job cards section
                window.scrollTo({ top: isMobile ? window.innerHeight * 0.4 : window.innerHeight * 0.7, behavior: 'smooth' });
                ...
                const applyBtn = document.getElementById("demo-job-apply-btn");
                if (applyBtn) {
                    applyBtn.style.transition = "all 0.4s ease";
                    applyBtn.style.transform = "scale(1.05)";
                    applyBtn.style.boxShadow = "0 0 35px rgba(6, 182, 212, 0.8), inset 0 0 15px rgba(6, 182, 212, 0.4)";
                    applyBtn.style.backgroundColor = "rgba(6, 182, 212, 0.3)";

                    // Highlight time
                    await new Promise(r => setTimeout(r, 1200));

                    // Simulate the physical press down
                    applyBtn.style.transition = "all 0.1s ease";
                    applyBtn.style.transform = "scale(0.95)";
                    await new Promise(r => setTimeout(r, 150));

                    applyBtn.click();
                    setTimeout(() => {
                        window.location.href = "/jobs/apply?role=1&demo=1";
                    }, 300);
                }
            };
            runDemo();
                    ...
                    <div>
                        <div
                            ...
                            onScroll={() => {
                                ...
                                const cardWidth = el.scrollWidth / ROLES.length;
                                const idx = Math.round(el.scrollLeft / cardWidth);
                                setMobileActiveIdx(Math.min(idx, ROLES.length - 1));
                            }}
                        >
                            {ROLES.map((role, i) => {
                                const localized = t.roles[role.id] as { title: string; description: string };
                                return (
                                    <div key={role.id} className="mobile-swipe-card">
                                        <div className="rounded-3xl bg-white/[0.03] border border-white/[0.08] p-5 h-full flex flex-col">
                                            {/* Header */}
                                            <div className="flex items-start gap-3 mb-3">
                                                <div className="w-10 h-10 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-center text-xl shrink-0">
                                                    {getRoleIcon(role.id)}
                                                </div>
                                            ...
                                            {/* Expanded Details */}
                                            {expandedMobileIdx === i && (() => {
                                                const full = t.roles[role.id] as { responsibilities?: string[]; requirements?: string[] };
                                                return (
                                                    <div className="mb-3 flex flex-col gap-3 border-t border-white/[0.06] pt-3">
                                                        {full.responsibilities && (
                                                            <div>
                                                                <h3 className="text-[10px] font-mono text-cyan-400/80 uppercase tracking-widest mb-1.5 flex items-center gap-1">
                                                                    <span className="w-3 h-px bg-cyan-400/50" />
                                                                    {t.respTitle}
                                                                </h3>
                                                                <ul className="space-y-1">
                                                                    {full.responsibilities.map((item, idx) => (
                                                                        <li key={idx} className="text-[11px] text-white/55 flex items-start gap-2">
                                                                            <span className="text-violet-500/70 mt-[1px] shrink-0 text-[10px]">▹</span>
                                                        ...
                                                        {full.requirements && (
                                                            <div>
                                                                ...
                                                                <ul className="space-y-1">
                                                                    {full.requirements.map((item, idx) => (
                                                                        <li key={idx} className="text-[11px] text-white/55 flex items-start gap-2">
                                                                            <span className="text-violet-500/70 mt-[1px] shrink-0 text-[10px]">▹</span>
                                                                            <span className="leading-relaxed">{item}</span>
                                                                        </li>
                                            ...
                                            {/* Buttons Row */}
                                            <div className="flex items-center gap-0 w-full p-1 bg-white/[0.02] border border-white/5 rounded-[18px]">
                                                {/* View Details Button */}
                                                <button
                                                    onClick={() => setExpandedMobileIdx(expandedMobileIdx === i ? null : i)}
                                                    className="flex-1 py-2 rounded-l-[14px] text-[11px] font-semibold text-white/60 hover:text-white hover:bg-white/[0.04] transition-all"
                                                >
                                                    {expandedMobileIdx === i ? t.hideDetails : t.viewDetails}
                                                </button>
                                                <div className="w-px h-5 bg-white/10" />
                                                {/* Apply Button */}
                                                <button
                                                    id={i === mobileActiveIdx ? "demo-job-apply-btn" : undefined}
                                                    onClick={() => {
                                                        const demoParam = typeof window !== "undefined" && window.location.search.includes("demo=1") ? "&demo=1" : "";
                                                        router.push(`/jobs/apply?role=${role.id}${demoParam}`);
                                                    }}
                                                    className="flex-1 py-2 rounded-r-[14px] text-[11px] font-bold uppercase tracking-widest bg-cyan-500/10 text-cyan-300 hover:bg-cyan-500/20 transition-all"
                                                >
                                                    {t.applyNow}
                                                </button>
                                            </div>
                        ...
                        {/* Dot pagination + swipe hint */}
                        <div className="flex flex-col items-center gap-3 mt-4">
                            <div className="flex gap-1.5">
                                {ROLES.map((_, i) => (
                                    <div
                                        key={i}
                                        className={`h-1.5 rounded-full transition-all duration-300 ${i === mobileActiveIdx ? 'bg-violet-400 w-6' : 'bg-white/15 w-1.5'}`}
                                    />
                                ))}
                            </div>
                            <div className="flex items-center gap-2 text-white/20 text-[11px] font-medium tracking-wider">
