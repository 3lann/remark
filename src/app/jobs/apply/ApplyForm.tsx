...
export default function ApplyForm() {
    ...
    const triggerAssistant = (key: string, textEn: string, textAr: string) => {
        if (isDemoMode) return; // Disable hover tips during demo tour
        ...
        if (typeof window !== "undefined" && window.location.search.includes("demo=1")) {
            ...
            const runDemo = async () => {
                ...
                window.dispatchEvent(new CustomEvent("assistant-message", {
                    detail: { text: "الآن وصلنا لصفحة التقديم! دعني أريك كيف نملأ البيانات بسرعة... ✍️", delay: 1000 }
                }));
                await new Promise(r => setTimeout(r, 3200));

                setForm(f => ({ ...f, fullName: "أحمد العراقي", phone: "+964 770 123 4567", email: "ahmed.demo@remark.iq", city: "بغداد" }));
                window.dispatchEvent(new CustomEvent("assistant-message", {
                    detail: { text: "أولاً: معلوماتك الشخصية لتسهيل التواصل معك 📞", delay: 0 }
                }));
                ...
                setForm(f => ({ ...f, yearsExperience: "3", strongestSkill: "إدارة الحملات والتسويق", expectedSalary: "يحدد بعد المقابلة" }));
                window.dispatchEvent(new CustomEvent("assistant-message", {
                    detail: { text: "ثانياً: مهارتك وخبراتك التي تجعلك الشخص المثالي لنا ✨", delay: 0 }
                }));
                await new Promise(r => setTimeout(r, 2600));
                ...
                setForm(f => ({ ...f, portfolioUrl: "https://dribbble.com/ahmed", whyJoin: "أنا مهتم جداً بفرصة العمل معكم والمساهمة في بناء مسارات تقنية حديثة ومبدعة!", workType: "Remote" }));
                window.dispatchEvent(new CustomEvent("assistant-message", {
                    detail: { text: "ثالثاً: أعمالك السابقة ورسالتك لتبهرنا باحترافيتك 🚀", delay: 0 }
                }));
                ...
                // Ask the user to click submit
                window.dispatchEvent(new CustomEvent("assistant-message", {
                    detail: { text: "أخيراً، قم برفع سيرتك الذاتية (CV)، ثم اضغط على زر الإرسال! 🎉", delay: 0 }
                }));
                await new Promise(r => setTimeout(r, 3200));

                const submitBtn = document.getElementById("demo-apply-submit");
                if (submitBtn) {
                    submitBtn.style.transition = "all 0.4s ease";
                    submitBtn.style.transform = "scale(1.05)";
                    submitBtn.style.boxShadow = "0 0 35px rgba(139, 92, 246, 0.9), inset 0 0 15px rgba(139, 92, 246, 0.4)";
                    submitBtn.style.backgroundColor = "rgba(139, 92, 246, 0.3)";

                    await new Promise(r => setTimeout(r, 1200));

                    // Simulate the physical press down
                    submitBtn.style.transition = "all 0.1s ease";
                    submitBtn.style.transform = "scale(0.95)";
                    await new Promise(r => setTimeout(r, 150));

                    submitBtn.click();

                    // Fallback to guarantee navigation in demo mode if form validation strictly blocks the click
                    setTimeout(() => {
                        window.location.href = "/jobs/success?demo=1";
                    }, 300);
                }
            };
            runDemo();
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            formData.append("years_experience", form.yearsExperience || "0");
            formData.append("strongest_skill", form.strongestSkill);
            formData.append("expected_salary", form.expectedSalary);
            formData.append("portfolio_url", form.portfolioUrl);
            formData.append("why_join", form.whyJoin);
            formData.append("work_type", form.workType);
            ...
            // Send directly to our proxy which forwards to n8n
            const response = await fetch("/api/notify-n8n", {
                method: "POST",
                body: formData, // passing FormData allows binary file uploads directly to n8n
            });

            if (!response.ok) {
                throw new Error("Failed to deliver submission to n8n.");
            }

            setStatus("success");
            router.push(isDemoMode ? "/jobs/success?demo=1" : "/jobs/success");
        } catch (err: unknown) {
            setStatus("error");
