...
export default function GlassCard({
    ...
    const isInteractive = hoverable && !isMobile;
    ...
    return (
        <div
            ...
            className={`
                relative overflow-hidden rounded-[2.5rem] border border-white/5 transition-all duration-500
                ${isMobile ? 'bg-white/[0.04]' : 'bg-white/[0.02] backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.2)]'}
                ${hoverable ? "cursor-pointer group hover:border-white/10" : ""}
                ${className}
            `}
            style={style}
        >
            {/* Hexagon Pattern Overlay — desktop only */}
            {!isMobile && (
                <div
                    ...
                    style={{
                        ...
                        opacity: (hoverable && isFocused) ? GLASSCARD_CFG.PATTERN_OPACITY + GLASSCARD_CFG.HOVER_OPACITY_DELTA : GLASSCARD_CFG.PATTERN_OPACITY,
                        mixBlendMode: "plus-lighter",
                    }}
                />
            )}

            {/* Spotlight Border Follow Effect — desktop only */}
            {!isMobile && hoverable && shimmer && (
                <div
                    className="pointer-events-none absolute -inset-px opacity-100 transition duration-300 z-[0]"
                    style={{
                        background: `radial-gradient(400px circle at ${shimmer.x}px ${shimmer.y}px, ${spotlightColor}, transparent 40%)`,
                    }}
                />
            )}

            {/* Top shimmer sweep on hover — desktop only */}
            {!isMobile && hoverable && (
                <span className="
                    pointer-events-none absolute top-0 left-0 right-0 h-px
                    bg-gradient-to-r from-transparent via-white/50 to-transparent
                    translate-x-[-100%] group-hover:translate-x-[100%]
                    transition-transform duration-[1.5s] ease-in-out z-[0]
                " />
            )}
