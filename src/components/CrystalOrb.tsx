...
export default function CrystalOrb({ size = 260, className = "", onStateChange }: CrystalOrbProps) {
    ...
    const springCfg = { stiffness: 120, damping: 28, mass: 0.5 };
    ...
    /* ── Mobile: auto-trigger spin once after 5 seconds ── */
    const hasAutoFired = useRef(false);
    useEffect(() => {
        if (!isMobile || hasAutoFired.current) return;
    ...
    return (
        <motion.div ref={containerRef}
            className={`relative select-none ${className} ${isMobile ? 'pointer-events-none' : 'cursor-pointer'}`}
            style={isMobile
                ? { width: size, height: size, touchAction: "none" }
                : { width: size, height: size, perspective: 800, rotateX, rotateY, transformStyle: "preserve-3d" as const, touchAction: "none" }
            }
            ...
            {/* Outer glow — mobile uses gradient only (no blur filter), desktop uses blur */}
            {!isMobile && <div className="absolute inset-0 rounded-full crystal-outer-pulse-element" style={{
                background: isSpin
            ...
            {/* Mobile outer glow — radial gradient, NO blur filter */}
            {isMobile && <div className="absolute inset-0 rounded-full" style={{
                background: isSpin
                    ? `radial-gradient(circle, rgba(${RGB.violetAccent},0.35) 0%, rgba(${RGB.accentBlue},0.15) 35%, transparent 60%)`
                    : `radial-gradient(circle, rgba(${RGB.accentBlue},0.20) 0%, rgba(${RGB.slateBlue},0.08) 40%, transparent 60%)`,
                transform: 'scale(1.5)',
                transition: gT,
            }} />}

            {/* Secondary halo — desktop only */}
            {!isMobile && <div className="absolute inset-0 rounded-full" style={{
                background: isSpin
            ...
            {/* Cursor-tracking glow (desktop only since no cursor on mobile) */}
            {!isMobile && (
            ...
            {!isMobile && <div className="absolute left-1/2 -translate-x-1/2" style={{
            ...
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="relative z-10" style={{
                filter: isMobile
                    ? 'none'
                    : `drop-shadow(0 0 ${isSpin ? 45 : 30}px rgba(${isSpin ? RGB.violetAccent : RGB.accentBlue},${isSpin ? 0.55 : 0.35}))`,
                transform: `rotate(${currentRotation}deg)`,
                transition: isMobile
                    ? (animState === "spinning" ? `transform ${TIMING.spinDuration}ms ${EASING.spin}` : 'none')
                    : (animState === "spinning"
                ...
                <defs>
                    ...
                    <radialGradient id={`${uid}-inner`} cx="35%" cy="28%" r="70%">
                        <stop offset="0%" stopColor={`rgba(${RGB.highlight},${isMobile ? 0.15 : 0.30})`} />
                    ...
                    <linearGradient id={`${uid}-face`} x1="0%" y1="0%" x2="100%" y2="100%">
                        <stop offset="0%" stopColor={`rgba(${RGB.highlight},${isMobile ? 0.45 : 0.25})`} />
                        <stop offset="40%" stopColor={`rgba(${RGB.accentBlue},${isMobile ? 0.30 : 0.15})`} />
                        <stop offset="100%" stopColor={`rgba(${RGB.slateBlue},${isMobile ? 0.18 : 0.08})`} />
                    </linearGradient>
                    <radialGradient id={`${uid}-spec`} cx="28%" cy="22%" r="60%">
                        <stop offset="0%" stopColor={`rgba(255,255,255,${isMobile ? 0 : 0.45})`} />
                        <stop offset="40%" stopColor={`rgba(${RGB.highlight},${isMobile ? 0 : 0.12})`} />
                ...
                <g clipPath={`url(#${uid}-clip)`}>
                    ...
                    {!isMobile && <circle cx={half * 0.9} cy={half * 0.85} r={half * 0.7} fill={`url(#${uid}-ctr)`} />}
                    ...
                    {cells.map((cell, i) => {
                        ...
                        if (isMobile) {
                            // Mobile: sharp cells with shimmer animation + subtle highlights
                            return (
                                <g key={i}>
                                    <polygon points={hexPts(cell.cx, cell.cy, cellR * 0.95)} fill="none"
                                        stroke={`rgba(${RGB.accentBlue},${(0.30 * (1 - cell.dist * 0.3)).toFixed(3)})`}
                                        strokeWidth={1.2} />
                                    <polygon points={hexPts(cell.cx, cell.cy, ir)} fill={`url(#${uid}-face)`}
                                        opacity={cell.depth * bri} className="crystal-cell"
                                        style={{ animationDelay: `${cell.delay}s` }} />
                                    {i % 3 === 0 && <polygon points={hexPts(cell.cx, cell.cy, ir * 0.5)}
                                        fill={`rgba(${RGB.highlight},${(0.08 * bri).toFixed(3)})`}
                                        className="crystal-cell" style={{ animationDelay: `${cell.delay + 2}s` }} />}
                                </g>
                            );
                        }

                        // Desktop: full rendering with overlays, highlights, sparkles
                    ...
                    {/* Inner lighting overlay — on both mobile and desktop */}
                    ...
                    <circle cx={half} cy={half} r={half - 2} fill="none"
                        stroke={isMobile ? `rgba(${RGB.accentBlue},0.30)` : (isSpin ? `rgba(${RGB.violetAccent},0.45)` : `rgba(${RGB.accentBlue},0.22)`)}
                        strokeWidth={isMobile ? 1.5 : (isSpin ? 3 : 2)}
                        style={isMobile ? {} : { transition: `stroke ${TIMING.glowTransition}ms ${EASING.glow}, stroke-width ${TIMING.glowTransition}ms ${EASING.glow}` }} />
                    {!isMobile && isSpin && <circle cx={half} cy={half} r={half - 4} fill="none"
                    ...
                    {!isMobile && <circle cx={half} cy={half} r={half - 6} fill="none"
