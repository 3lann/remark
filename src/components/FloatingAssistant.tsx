...
export default function FloatingAssistant() {
                ...
                if (!prev) {
                    ...
                    if (messages.length === 0) {
                        setMessages([{
                            ...
                            text: isAr
                                ? "مرحباً! أنا مساعد ريمارك، هنا لمساعدتك 😊"
                                : "Hello! 👋 I'm Remark Assistant. Happy to help with any questions about our company or positions. How can I help?",
                        }]);
                    }
                    return true;
                } else {
    ...
    useEffect(() => {
        ...
        if (!hasShownWelcome) {
            const timer = setTimeout(() => {
                ...
                setBubbleText(isAr ? "مرحباً! أنا مساعد ريمارك، هنا لمساعدتك 😊" : "Hi! I'm Remark Assistant 😊");
                setWelcomeShow(true);
                setSpinKey(k => k + 1);
    ...
    const send = useCallback(async () => {
        ...
        try {
            ...
            reply = data.reply || getMockReply(txt, lang);
        } catch {
            // Fallback to mock if API fails
            reply = getMockReply(txt, lang);
        }

        setTyping(false);
        setMessages(p => [...p, {
            id: ++idRef.current, text: reply, sender: "assistant", ts: new Date(),
        }]);
    }, [input, lang]);
    ...
    return (
        <>
                ...
                {welcomeShow && (
                    <motion.div
                        ...
                        className={`fixed z-[9998] max-w-[280px] cursor-pointer ${showBottom
                            ? (isAr ? "bottom-[84px] left-4" : "bottom-[84px] right-4")
                            : `top-[96px] right-4`
                            }`}
                    ...
                    >
                        {showBottom ? (
                            ...
                            <div className={`relative px-5 py-3.5 rounded-2xl text-[13px] font-medium text-white/95 leading-relaxed shadow-[0_8px_32px_rgba(0,0,0,0.2)] border border-white/10 hover:border-white/20 transition-colors ${isAr ? 'rounded-tl-md' : 'rounded-tr-md'}`}
                                ...
                                {/* Caret Top */}
                                <div className={`absolute -top-[6px] right-4 w-3 h-3 rotate-45 border-l border-t border-white/10`}
                                    style={{ background: "rgba(10, 15, 30, 0.9)", ...(isMobile ? {} : { backdropFilter: "blur(24px)" }) }} />
                            </div>
                        )}
                    </motion.div>
                )}
            ...
            <AnimatePresence>
                {showBottom && (
                    <motion.div
                        ...
                        className={`fixed z-[9999] bottom-4 ${isAr ? 'left-4' : 'right-4'}`}
            ...
            <AnimatePresence>
                {chatOpen && (
                    <motion.div
                        initial={{ opacity: 0, y: showBottom ? 24 : -24, scale: 0.94 }}
                        animate={{ opacity: 1, y: 0, scale: 1 }}
                        exit={{ opacity: 0, y: showBottom ? 16 : -16, scale: 0.96 }}
                        transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
                        className={`fixed z-[9998] w-[360px] max-w-[calc(100vw-32px)] flex flex-col overflow-hidden ${showBottom
                            ? `bottom-4 ${isAr ? 'left-4' : 'right-4'}`
                            : `top-[96px] ${isAr ? "left-[max(16px,calc(50vw-560px))]" : "right-[max(16px,calc(50vw-560px))]"}`
                            }`}
                        style={{
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3 px-5 py-4 shrink-0" style={{
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="text-sm font-semibold text-white/90">{isAr ? "مساعد ريمارك" : "Remark Assistant"}</div>
